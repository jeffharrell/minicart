...
var defaults = module.exports = {
    ...
    template: '<%	var items = cart.items();	var settings = cart.settings();%><form method="post" class="<% if (!items.length) { %>minicart-empty<% } %>" action="<%= config.action %>" target="<%= config.target %>">	<ul>		<% for (i= 0, len = items.length; i < len; i++) { %>			<li class="minicart-item">				<a class="minicart-name" href="<%= items[i].link %>"><%= items[i].get("item_name") %></a>				<ul class="minicart-attributes">					<% if (items[i].get("item_number")) { %>						<li><%= items[i].get("item_number") %></li>					<% } %>					<% for (var options = items[i].options(), j = 0, len2 = options.length; j < len2; j++) { %>						<li><%= options[j].key %>: <%= options[j].value %></li>					<% } %>				</ul>				<input class="minicart-quantity" data-minicart-idx="<%= i %>" name="quantity_<%= i %>" value="<%= items[i].get("quantity") %>" autocomplete="off" />				<input class="minicart-remove" data-minicart-idx="<%= i %>" type="button" />				<span class="minicart-price"><%= items[i].total({ format: true }) %></span>				<input type="hidden" name="item_name_<%= i %>" value="<%= items[i].get("item_name") %>" />				<input type="hidden" name="item_number_<%= i %>" value="<%= items[i].get("item_number") %>" />				<input type="hidden" name="amount_<%= i %>" value="<%= items[i].get("amount") %>" />			</li>		<% } %>	</ul>	<div>		<div class="minicart-subtotal" onclick="paypal.minicart.toggle()">			<%= config.strings.subtotal %> <%= cart.total({ format: true, currencyCode: true }) %>		</div>		<input class="minicart-submit" type="submit" value="<%= config.strings.button %>" data-test-processing="<%= config.strings.processing %>" />	</div>	<input type="hidden" name="cmd" value="_cart" />	<input type="hidden" name="upload" value="1" />	<input type="hidden" name="bn" value="MiniCart_AddToCart_WPS_US" />	<% for (var key in settings) { %>		<input type="hidden" name="<%= key %>" value="<%= settings[key] %>" />	<% } %></form>',
